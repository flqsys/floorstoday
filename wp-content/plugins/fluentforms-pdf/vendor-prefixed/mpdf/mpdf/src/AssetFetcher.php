...
use FluentPdf\Vendor\Mpdf\File\LocalContentLoaderInterface;
use FluentPdf\Vendor\Mpdf\File\StreamWrapperChecker;
use FluentPdf\Vendor\Mpdf\Http\ClientInterface;
use FluentPdf\Vendor\Mpdf\Log\Context as LogContext;
use FluentPdf\Vendor\Mpdf\PsrHttpMessageShim\Request;
use FluentPdf\Vendor\Mpdf\PsrLogAwareTrait\PsrLogAwareTrait;
use FluentPdf\Vendor\Psr\Log\LoggerInterface;

class AssetFetcher implements \FluentPdf\Vendor\Psr\Log\LoggerAwareInterface
{
	...
	public function fetchDataFromPath($path, $originalSrc = null)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \FluentPdf\Vendor\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}
	...
	public function fetchRemoteContent($path)
	{
		...
		try {
			...
			/** @var \FluentPdf\Vendor\Mpdf\PsrHttpMessageShim\Response $response */
			...
			if (!str_starts_with((string) $response->getStatusCode(), '2')) {
				...
				if ($this->mpdf->debug) {
					throw new \FluentPdf\Vendor\Mpdf\MpdfException($message);
				}
			...
			if ($this->mpdf->debug) {
				throw new \FluentPdf\Vendor\Mpdf\MpdfException($message, 0, E_ERROR, null, null, $e);
			}
