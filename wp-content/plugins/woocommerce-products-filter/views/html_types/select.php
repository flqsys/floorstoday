		...
		<?php if ( ! empty( $childs ) ) : ?>
			<?php foreach ( $childs as $term ) : ?>
				...
				if ( $show_count and $count == 0 and ! in_array( $term['slug'], $current_request ) ) :
					?>
					disabled=""<?php endif; ?> value="<?php echo esc_attr( $term['slug'] ); ?>" <?php selected( in_array( $term['slug'], $current_request ) ); ?> class="woof-padding-<?php echo esc_attr( $level ); ?>"><?php echo wp_kses_post( str_repeat( '&nbsp;&nbsp;&nbsp;', $level ) ); ?>
				<?php
