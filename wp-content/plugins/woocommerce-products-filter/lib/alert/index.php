...
add_action(
	...
	function () {
		...
		add_action(
			"wp_ajax_{$slug}_dismiss_rate_alert",
			function () use ( $slug ) {
				if ( ! isset( $_REQUEST['woof_alert_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['woof_alert_nonce'] ) ), 'alert_nonce' ) ) {
					die( '0' );
				}
		...
		add_action(
			"wp_ajax_{$slug}_later_rate_alert",
			function () use ( $slug ) {
				if ( ! isset( $_REQUEST['woof_alert_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['woof_alert_nonce'] ) ), 'alert_nonce' ) ) {
					die( '0' );
				}
		...
		add_action(
			...
			<div class="notice notice-info" id="pn_<?php echo esc_attr( $slug ); ?>_ask_favour" style="position: relative;">
				...
				<p>
					<?php
						/* translators: %s: website/marketplace name */
						printf( esc_html__( 'That\'s awesome! Could you please do us a BIG favor and give it a 5-star rating on %s to help us spread the word and boost our motivation?', 'woocommerce-products-filter' ), esc_html( $on ) )
					?>
				</p>
