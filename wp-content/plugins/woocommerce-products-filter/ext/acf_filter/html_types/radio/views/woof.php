...
?>
<div data-css-class="woof_acf_radio_container" class="woof_acf_radio_container woof_container woof_container_<?php echo esc_attr( $meta_key ); ?>  woof_container_<?php echo esc_attr( $meta_key ); ?> <?php echo esc_attr( WOOF_HELPER::generate_container_css_classes( $meta_key ) ); ?>">
				...
				<ul class="woof_list woof_list_radio">
					...
					foreach ( $all_options as $key => $option ) :
						...
						<li class="woof_term_<?php echo esc_attr( $key ); ?> ">
							<input type="radio" 
							<?php
							if ( ! $option['count'] and $key != $woof_value and $show_count ) :
								?>
								disabled=""<?php endif; ?> id="<?php echo esc_attr( 'woof_' . $key . '_' . $inique_id ); ?>" class="woof_radio_term woof_radio_term_<?php echo esc_attr( $key ); ?>" data-slug="<?php echo esc_attr( $key ); ?>" data-term-id="<?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $meta_key ); ?>" value="<?php echo esc_attr( $key ); ?>" <?php checked( $key == $woof_value ); ?> />
							<label class="woof_radio_label 
							<?php
							if ( $key == $woof_value ) :
								?>
								woof_radio_label_selected<?php endif; ?>" for="<?php echo esc_attr( 'woof_' . $key . '_' . $inique_id ); ?>"><?php echo wp_kses_post( wp_unslash( $option['name'] ) ); ?></label>

							<a href="#" rel="nofollow" data-name="<?php echo esc_attr( $meta_key ); ?>" data-term-id="<?php echo esc_attr( $key ); ?>" style="
							<?php
							if ( $key != $woof_value ) :
								?>
								display: none;<?php endif; ?>" class="woof_radio_term_reset  
								<?php
								if ( $key == $woof_value ) :
									?>
								woof_radio_term_reset_visible<?php endif; ?> woof_radio_term_reset_<?php echo esc_attr( $key ); ?>">
