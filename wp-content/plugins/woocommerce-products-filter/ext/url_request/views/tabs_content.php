			...
			<section>
				...
				<div class="woof-section-title">
					<div class="col-title">
						...
						<?php if ( empty( trim( get_option( 'permalink_structure' ) ) ) ) : ?>
							<br>
							<div class="woof__alert woof__alert-info2 woof_tomato">
								<?php esc_html_e( 'Permalinks should be enabled!', 'woocommerce-products-filter' ); ?>
							</div>
						<?php endif; ?>
							...
							<div class="select-wrap">
								<select name="woof_settings[woof_url_request][page_index]" class="chosen_select">
									<?php foreach ( $page_index as $key => $value ) : ?>
										<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $index == $key ); ?>><?php echo esc_html( $value ); ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
							...
							<div class="select-wrap">
								<select name="woof_settings[woof_url_request][index_deep]" class="chosen_select">
									<?php foreach ( $index_deeps as $key => $value ) : ?>
										<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $index_deep == $key ); ?>><?php echo esc_html( $value ); ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
								...
								<div>
									<select class='woof_seo_current_lang <?php echo esc_attr( $add_class ); ?>'>
										...
										foreach ( $langs as $lang ) {
											?>
											<option values='<?php echo esc_attr( $lang ); ?>'><?php echo esc_html( $lang ); ?></option>
											<?php
										}
										?>
									</select>
