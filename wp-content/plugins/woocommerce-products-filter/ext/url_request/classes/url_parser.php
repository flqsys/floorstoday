...
class WOOF_URL_PARSER {
	...
	public function url_process( $do, $WP, $extra_query_vars ) {
		...
		foreach ( get_taxonomies( array( 'object_type' => 'product' ), 'objects' ) as $taxonomy => $t ) {
	...
	public function get_request_uri() {
		...
		if ( isset( $_SERVER['REQUEST_URI'] ) ) {
			$uri = sanitize_text_field( wp_unslash( urldecode( $_SERVER['REQUEST_URI'] ) ) );// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		}
		return apply_filters( 'woof_override_seo_request_uri', $uri );
	}

	public function get_post() {
        // phpcs:ignore WordPress.Security.NonceVerification.Missing -- This is a utility method; nonce verification is handled by the calling AJAX handler.
