...
final class WOOF_FRONT_BUILDER_OPTIONS_SECTIONS_LAYOUT {
	...
	public function init() {
		add_action(
			'wp_ajax_woof_form_builder_get_section_layout_options',
			function () {
				if ( ! wp_verify_nonce( WOOF_REQUEST::get( 'woof_front_builder_nonce' ), 'front_builder_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					return false;
				}
				die( json_encode( $this->get_options( sanitize_text_field( wp_unslash( $_REQUEST['name'] ) ), sanitize_key( wp_unslash( $_REQUEST['section_key'] ) ) ) ) );
			}
		);
		add_action( 'wp_ajax_woof_front_builder_save_section_layout_option', array( $this, 'save_options' ) );
		if ( $this->demo ) {
			add_action(
				'wp_ajax_nopriv_woof_form_builder_get_section_layout_options',
				function () {
					if ( ! wp_verify_nonce( WOOF_REQUEST::get( 'woof_front_builder_nonce' ), 'front_builder_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
						return false;
					}
	...
	public function save_options() {
		...
		$section_key = sanitize_key( wp_unslash( $_REQUEST['section_key'] ) );
	...
	public function get_options_key_value( $name, $section_key = '' ) {
		$res = array();
