...
final class WOOF_SD extends WOOF_EXT {
	...
	private $db                                = null;
	private $table                             = null;
	public $outer_templates                    = array();
	public $outer_ext_dir                      = '';
	public $outer_templates_dir                = '';
	public $outer_ext_link                     = '';
	public $outer_templates_link               = '';
	...
	private function init_outer_elements() {
		...
		if ( ! empty( $dirs ) ) {
			foreach ( $dirs as $dir_name ) {
				...
				$this->outer_templates[ $dir_name ] = array(
	...
	public function init() {
		...
		add_action(
			...
			function ( $is, $html_type, $tax_slug, $args ) use ( $self ) {
				if ( $self->html_type && $html_type === $self->html_type ) {
					if ( substr( $tax_slug, 0, strlen( $self->html_type ) ) === $html_type ) {
						...
						if ( isset( $this->outer_templates[ $type ] ) ) {
							...
							$file = $this->get_ext_path() . "templates/{$type}.html";
							if ( $template_num > 0 ) {
								$file = $this->get_ext_path() . "templates/{$type}-{$template_num}.html";
							}
						}
		...
		add_action(
			'wp_ajax_woof_sd_boot',
			function () {

				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}
				...
				if ( ! empty( $rows_raw ) ) {
					foreach ( $rows_raw as $rr ) {
						...
						$rows[ (string) $rr['id'] ] = array(
							'title'   => array( 'value' => $rr['title'] ),
							'type'    => array( 'value' => $rr['type'] ),
							// draw_row_actions is custom created function in js class Row
							'actions' => array(
								'value'        => '',
								'draw_content' => 'draw_row_actions',
								'classes'      => 'woof-sd-edit-row',
							),
						);
					}
				}
				...
				$data = array(
					'header' => array(
						array(
							'value'    => esc_html__( 'Title', 'woocommerce-products-filter' ),
							'width'    => '45%',
							'editable' => 1,
							'key'      => 'title',
							'action'   => 'woof_sd_change_title',
						),
						array(
							'value' => esc_html__( 'Type', 'woocommerce-products-filter' ),
							'width' => '30%',
							'key'   => 'type',
						),
						array(
							'value' => esc_html__( 'Actions', 'woocommerce-products-filter' ),
							'width' => '25%',
						),
					),
					'rows'   => $rows,
		...
		add_action(
			...
			function () {
				...
				if ( sanitize_key( wp_unslash( $_REQUEST['change_type'] ) ) ) {
		...
		add_action(
			'wp_ajax_woof_sd_change_template',
			function () {
				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}
		...
		add_action(
			'wp_ajax_woof_sd_change_demo_taxonomy',
			function () {
				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}

				$taxonomy = sanitize_key( wp_unslash( $_REQUEST['taxonomy'] ) );
				$id       = intval( $_REQUEST['id'] );
		...
		add_action(
			'wp_ajax_woof_sd_create_element',
			function () {
				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}

				echo json_encode( $this->create_element( sanitize_text_field( wp_unslash( $_REQUEST['title'] ) ) ) );
				exit;
			}
		);

		add_action(
			'wp_ajax_woof_sd_change_title',
			function () {
				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}
	...
	public function change_title( $id, $new_title ) {
		$this->db->update( $this->table, array( 'title' => $new_title ), array( 'id' => $id ) );
	}

	public function create_element( $title ) {
		$title = $title . ' ' . gmdate( 'd-m-Y H:i:s' );
		$this->db->insert(
			$this->table,
			array(
				'title' => $title,
	...
	public function wp_head() {
		// 27-06-2023 HUSKY calls such css always
	...
	public function woof_print_applications_tabs_content() {
		...
		// boot
		wp_enqueue_script( 'woof_sd_admin', $this->get_ext_link() . 'js/admin.js', array(), WOOF_VERSION );
		wp_enqueue_script( 'woof_sd_boot', $this->get_ext_link() . 'js/boot.js', array(), WOOF_VERSION );
		$this->script_loader_tag(); // !!

		wp_enqueue_style( 'woof_sd_checkbox', $this->get_ext_link() . 'css/elements/checkbox.css', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_sd_radio', $this->get_ext_link() . 'css/elements/radio.css', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_sd_switcher', $this->get_ext_link() . 'css/elements/switcher.css', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_sd_color', $this->get_ext_link() . 'css/elements/color.css', array(), WOOF_VERSION );

		if ( ! empty( $this->outer_templates ) ) {
			foreach ( $this->outer_templates as $key => $template ) {
				wp_enqueue_style( "woof_sd_{$key}", $template['styles'], array(), WOOF_VERSION );
			}
		}

		wp_enqueue_style( 'woof_sd_growl', $this->get_ext_link() . 'css/growl.css', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_sd_admin', $this->get_ext_link() . 'css/admin.css', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_sd_ranger', $this->get_ext_link() . 'css/ranger.css', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_sd_switcher23', $this->get_ext_link() . 'css/switcher.css', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_sd_table', $this->get_ext_link() . 'css/table.css', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_sd_popup', $this->get_ext_link() . 'css/popup.css', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_sd_tooltip', $this->get_ext_link() . 'css/tooltip.css', array(), WOOF_VERSION );
		...
		wp_localize_script(
			...
			array(
				...
				'lang'               => array(
					...
					'assign_terms_color' => esc_html__( 'To assign color value to terms select appropriate taxonomy here!', 'woocommerce-products-filter' ),
		...
		add_filter(
			...
			function ( $tag, $handle, $src ) {
				...
				if ( 'woof_sd_boot' === $handle ) {
					// phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript -- type="module" requires direct tag modification, not possible via wp_enqueue_script().
	...
	private function install() {
		...
		if ( $this->db->query( $sql ) === false ) {
			$answer = esc_html__( 'HUSKY cannot create database table for smart designer! Make sure that your mysql user has the CREATE privilege! Do it manually using your host panel and phpmyadmin!', 'woocommerce-products-filter' );
