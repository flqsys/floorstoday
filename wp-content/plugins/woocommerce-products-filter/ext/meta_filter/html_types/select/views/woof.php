...
?>
<div data-css-class="woof_meta_select_container" class="woof_meta_select_container woof_container woof_container_<?php echo esc_attr( $meta_key ); ?>  woof_container_<?php echo esc_attr( 'select_' . $meta_key ); ?> <?php echo esc_attr( WOOF_HELPER::generate_container_css_classes( $meta_key ) ); ?>">
				...
				<select id="<?php echo esc_attr( $meta_id ); ?>" class="woof_meta_select woof_meta_select_<?php echo esc_attr( $meta_key ); ?>" name="<?php echo esc_attr( 'select_' . $meta_key ); ?>">
					...
					<?php if ( ! empty( $all_options ) ) : ?>
						<?php foreach ( $all_options as $key => $option ) : ?>
							...
							<option 
							<?php
							if ( $show_count and $option['count'] == 0 and $key != $woof_value ) :
								?>
								disabled=""<?php endif; ?> value="<?php echo esc_attr( $key ); ?>" <?php selected( $key == intval( $woof_value ) ); ?>>
								<?php
