			...
			<section>
				...
				function woof_meta_print_li( $m, $meta_types ) {
					...
					<li class="woof_options_li">
							...
							<div class="select-wrap">
								<select name="woof_settings[meta_filter][<?php echo esc_attr( $m['meta_key'] ); ?>][search_view]" class="woof_meta_view_selector woof_width_99p">
									...
									foreach ( $meta_types as $key => $type ) :
										...
										?>
																				<option  <?php selected( $m['search_view'], $key ); ?> value="<?php echo esc_html( $key ); ?>" data-show-options="<?php echo esc_attr( ( $type['show_options'] ) ? 'yes' : 'no' ); ?>" data-hideif="<?php echo esc_html( implode( ',', $type['hide_if'] ) ); ?>" 
																				<?php
																				if ( in_array( $m['type'], $type['hide_if'] ) ) :
																					?>
						...
						?>
						<div class="woof_options_item_options" 
						<?php
						if ( ! $show_options ) :
							?>
							style="display:none;" <?php endif; ?> >
						...
						<div class="woof_options_item">
							<div class="select-wrap" 
							<?php
							if ( in_array( $m['search_view'], array( 'popupeditor', 'switcher' ) ) ) :
								?>
								style="display: none;"<?php endif; ?>>
