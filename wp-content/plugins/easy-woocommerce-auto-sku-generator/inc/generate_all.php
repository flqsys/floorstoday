<?php
/**
 * AJAX callback: bulk SKU generation for all products.
 *
 * Supports batch processing: the caller passes `batch_size` (1–50).
 * Response is JSON: { html: string, processed: int, has_more: bool }
 *
 * Backward-compatible: if the client sends no batch_size the behaviour
 * is identical to the previous single-item approach (batch_size = 1,
 * plain HTML response recognised by the old empty-string check).
 *
...
/**
 * Bulk generation AJAX handler for all products.
 *
 * @return void
 */
function load_posts_by_ajax_callback() {
	...
	while ( $query->have_posts() ) {
		...
		$sku_output  = BulkGenerationService::get_sku_output( $product_id, $checked, $auto_number, $auto_id, $number_dop, false, 'easy-woocommerce-auto-sku-generator' );
